...
from config import Key, Converter, Formatter
...
class Kline:
    ...
    def find_support_and_resistance(self):
        ...
        if zig.levels is None:
            print('df')
            percent = 0.5
            zig = zig_zag_cluster_levels(percent, None, 0.1, 31, 'Low')
...
selected_symbol = 'BTCGBP'
# '1m', '3m', '5m', '30m', '1h', '2h', '4h', '6h', '8h', '12h', '1d', '3d', '1M'
selected_interval = '15m'

info = market.info(selected_symbol)
tick = market.ticker(selected_symbol)
line = market.kline(selected_symbol, selected_interval)

line[line.columns[0:5]].to_csv(index=False,
                               path_or_buf=os.environ[
                                               'USERPROFILE'] + '\\Desktop\\' + selected_symbol + ' ' + selected_interval +
                                           '.csv')
